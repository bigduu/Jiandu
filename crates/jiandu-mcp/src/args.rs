...
pub const MEMORY_ACTIONS: [&str; 19] = [
    "session_read",
    "session_append",
    "session_replace",
    "session_clear",
...
pub enum MemoryArgs {
    ...
    /// Read lower-trust host-generated orientation for Global or the
    /// host-authorized Project. A missing snapshot is a normal cold state; use
    /// query/get and live tools for factual decisions.
    DreamRead {
        #[schemars(with = "DurableScopeSchema")]
        scope: String,
        #[serde(default)]
    ...
    /// Publish bounded Markdown synthesized by the host's own model. Jiandu
    /// rejects the result if canonical memory changed after synthesis began.
    DreamPublish {
        #[schemars(with = "DurableScopeSchema")]
        scope: String,
        #[schemars(
            ...
            description = "current_generation returned by dream_read before the host began synthesis."
        )]
        source_generation: String,
        #[schemars(
            length(min = 1, max = 12000),
            description = "Compact non-empty Markdown generated by the host; Jiandu makes no model call."
        )]
        content: String,
        #[serde(default)]
        #[schemars(
...
impl MemoryArgs {
    ...
    pub const fn action_name(&self) -> &'static str {
        match self {
            ...
            Self::DreamPublish { .. } => "dream_publish",
            Self::Write { .. } => "write",
            Self::Merge { .. } => "merge",
            Self::Split { .. } => "split",
